...
NumbatConvection::computeQpResidual()
{
  RealVectorValue velocity =
      -_permeability[_qp] / _viscosity * (_grad_pressure[_qp] + _density[_qp] * _gravity);
...
NumbatConvection::computeQpJacobian()
{
  RealVectorValue velocity =
      -_permeability[_qp] / _viscosity * (_grad_pressure[_qp] + _density[_qp] * _gravity);
  RealVectorValue dvelocity_dc = -_permeability[_qp] / _viscosity * _ddensity_dc[_qp] * _gravity;
...
NumbatConvection::computeQpOffDiagJacobian(unsigned int jvar)
{
  if (jvar == _pvar)
  {
    RealVectorValue dvelocity = -_permeability[_qp] / _viscosity * (_grad_phi[_j][_qp]);
