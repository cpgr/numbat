...
NumbatDarcy::computeQpResidual()
{
  return _grad_test[_i][_qp] *
         (_permeability[_qp] / _viscosity * (_grad_u[_qp] + _density[_qp] * _gravity));
}

Real
NumbatDarcy::computeQpJacobian()
{
  return _grad_test[_i][_qp] * (_permeability[_qp] / _viscosity * _grad_phi[_j][_qp]);
}
...
NumbatDarcy::computeQpOffDiagJacobian(unsigned int jvar)
{
  ...
  {
    return _grad_test[_i][_qp] *
           (_permeability[_qp] / _viscosity * _phi[_j][_qp] * _ddensity_dc[_qp] * _gravity);
  }
  else
    return 0.0;
}
